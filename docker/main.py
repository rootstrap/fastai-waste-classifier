...
def process_image(upload):
    ...
    if 'predicted' in prediction.keys():
        response=f"Classified as **{prediction['predicted'].upper()}** with probability **{prediction['probability']}**"
    else:
...
col1.write('''The aim is to build a model for waste classification that identifies among the different classes:
- cardboards\n
- compost\n
- glass\n
- metal\n
